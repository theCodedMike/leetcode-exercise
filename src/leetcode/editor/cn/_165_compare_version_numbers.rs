...
//
//
// Related Topics Two Pointers String 👍 2028 👎 2516
...
impl Solution {
    pub fn compare_version(version1: String, version2: String) -> i32 {
        let mut v1 = version1.split_terminator(".");
        let mut v2 = version2.split_terminator(".");

        loop {
            match (v1.next(), v2.next()) {
                (None, None) => break,
                (Some(s1), None) => {
                    let i1 = s1.parse::<i32>().expect("Failed to parse s1");
                    if i1 != 0 {
                        return 1;
                    }
                }
                (None, Some(s2)) => {
                    let i2 = s2.parse::<i32>().expect("Failed to parse s2");
                    if i2 != 0 {
                        return -1;
                    }
                }
